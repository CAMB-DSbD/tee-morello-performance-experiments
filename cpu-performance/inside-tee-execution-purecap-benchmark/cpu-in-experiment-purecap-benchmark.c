 ...
 *              : Carlos Molina-Jimenez carlos.molina@cl.cam.ac.uk
 *              :
 * Title        : cpu-in-experiment-benchmarkABI.c 
 *              :
 * Description  : The cpu-in-experiment-benchmarkABIs.c programme carries out tests
 ...
 *              :
 * Compile      :
cm770@morello-camb-3:$ cc -g -O2 -Wall -march=morello+c64 -mabi=purecap-benchmark -o cpu-in-experiment-benchmarkABI cpu-in-experiment-benchmarkABI.c -L. -lm
clang-14: warning: Using c64 in the arch string is deprecated. The CPU mode should be inferred from the ABI. [-Wdeprecated]
cpu-in-experiment-benchmarkABI.c:73:29: warning: unused variable 'result' [-Wunused-variable]
            volatile double result = sin(i) * cos(i) * tan(i) * sqrt(i) * log(i + 1);
                            ^
cpu-in-experiment-benchmarkABI.c:90:26: warning: variable 'result' set but not used [-Wunused-but-set-variable]
            volatile int result = i * i / (i + 1);
                         ^
cpu-in-experiment-benchmarkABI.c:111:28: warning: variable 'result' set but not used [-Wunused-but-set-variable]
            volatile float result = (float)i / (i + 1) * (float)i;
                           ^
3 warnings generated.
 
 * run          : proccontrol -m cheric18n -s enable ./cpu-in-experiment-benchmarkABI         
 * 
 * 
*/
...
int main() {
    ...
    FILE *log_file = fopen("cpu-in-experiment-benchmarkABI-results.csv", "w");
    if (log_file == NULL) {
